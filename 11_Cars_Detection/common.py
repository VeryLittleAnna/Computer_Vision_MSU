from рабочая_detection_and_metrics import get_cls_model, get_detection_model, calc_auc, nms, get_detections
...
def calc_detector_auc(img_dir, gt_path, apply_nms=False):
    ...
    if apply_nms:
        # pred = nms(pred, iou_thr=1)
        # print(f"AUC:{calc_auc(pred, images_detection_only_bboxes)}")
        pred = nms(pred)
    ans = calc_auc(pred, images_detection_only_bboxes)
    print(f"AUC result:{ans}")
    return ans
