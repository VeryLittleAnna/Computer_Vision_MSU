...
def check_test(data_dir):
    ...
    print("-> ", b_row, b_col, r_row, r_col, " vs ", gt_b_row, gt_b_col, gt_r_row, gt_r_col)
    diff = abs(b_row - gt_b_row) + abs(b_col - gt_b_col) + \
        abs(r_row - gt_r_row) + abs(r_col - gt_r_col)
        
    print("	", diff)
    dif_file = open('diff1.txt', "a")
    print("->", diff, file=dif_file)
    print(b_row - gt_b_row, b_col - gt_b_col, r_row - gt_r_row, r_col - gt_r_col, file=dif_file)
    dif_file.close()
